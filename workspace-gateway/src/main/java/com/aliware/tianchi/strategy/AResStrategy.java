...
public class AResStrategy extends AbstractStrategy {
    ...
    public int select(URL url, Invocation invocation) {
        ...
        double k1 = Math.log(rand.nextDouble()) / (smallActiveCount);
        double k2 = Math.log(rand.nextDouble()) / (mediumActiveCount << 1);
        double k3 = Math.log(rand.nextDouble()) / (largeActiveCount * 3);
